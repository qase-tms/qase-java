# Test Steps in TestNG

This guide covers how to define and report test steps for detailed execution tracking in Qase using the TestNG reporter.
...
- `QaseListener` is not registered via `org.testng.ITestNGListener` SPI file
- Listener is not configured in test class or testng.xml

**Solutions:**

1. Verify `QaseListener` is registered via `org.testng.ITestNGListener` SPI file in `META-INF/services`.
2. Alternatively, add `@Listeners(QaseListener.class)` annotation to test class.
3. Or configure in `testng.xml`:
   ```xml
   <listeners>

...
- **failed** — the test executed correctly but the assertion did not pass (e.g., `Assert.assertEquals(expected, actual)` mismatch). This indicates a bug in the application under test.
...
The reporter detects assertion libraries automatically, including TestNG (`org.testng`), AssertJ (`org.assertj`), and OpenTest4J (`org.opentest4j`).

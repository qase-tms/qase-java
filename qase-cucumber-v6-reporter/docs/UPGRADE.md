# Upgrade Guide: Cucumber 6 Reporter

This guide covers migration from Qase Java Reporter v3 to v4 for the Cucumber 6 reporter.
...
1. **Maven artifact ID renamed**: `qase-cucumber6-jvm` → `qase-cucumber-v6-reporter`
...
5. **New `Qase` utility class**: `import io.qase.cucumber6.Qase` for attachments and comments
...
**Gradle Before (v3):**
```groovy
testImplementation 'io.qase:qase-cucumber6-jvm:3.2.1'
```

**Gradle After (v4):**
```groovy
testImplementation 'io.qase:qase-cucumber-v6-reporter:4.1.34'
```
...
| `import io.qase.api.services.Attachments;` | Use `io.qase.cucumber6.Qase` instead |
...
- **v3:** `QaseEventListener implements Formatter` (Cucumber 6 API: `cucumber.api.event.*`)
- **v4:** `QaseEventListener implements ConcurrentEventListener` (Cucumber 6 API: `io.cucumber.plugin.event.*`)
...
- [ ] Maven/Gradle dependency updated from `qase-cucumber6-jvm` to `qase-cucumber-v6-reporter` v4.1.34
...
- Ensure `import io.qase.cucumber6.Qase;` is used
...
- Verify Cucumber version compatibility (Cucumber 6.x)
...
- Ensure you're using qase-cucumber-v6-reporter version 4.1.5 or later

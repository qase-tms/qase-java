# Upgrade Guide: JUnit 4 Reporter

This guide covers migration from Qase Java Reporter v3 to v4 for the JUnit 4 reporter.
...
|---|---|
| v4.0.0 - v4.1.34 (current) | New architecture, v2 API, qase.config.json, new annotations |
...
1. **Maven artifact ID renamed**: `qase-junit4` → `qase-junit4-reporter`
...
5. **New `Qase` utility class**: `import io.qase.junit4.Qase` for attachments and comments
...
**Gradle After (v4):**
```groovy
testImplementation 'io.qase:qase-junit4-reporter:4.1.34'
```
...
| `import io.qase.api.services.Attachments;` | Use `io.qase.junit4.Qase` instead |
...
### JUnit 4 Aspect Module Merged

- **v3:** Separate `qase-junit4-aspect` module existed alongside `qase-junit4`
- **v4:** `qase-junit4-aspect` module removed - all functionality merged into `qase-junit4-reporter`

**Action required:** Remove the old `qase-junit4-aspect` dependency if you had it in v3:

```xml
<!-- Remove this from pom.xml if present -->
<dependency>
    <groupId>io.qase</groupId>
...
### JUnit 4 Listener Registration

- **v3:** `QaseListener` class in `qase-junit4` module
- **v4:** `QaseJunit4Aspects` handles listener registration via AspectJ

**AspectJ requirement:** Ensure aspectjweaver is configured in your build. The reporter uses AspectJ for both @Step annotation support and listener registration.
...
- [ ] Maven/Gradle dependency updated to `qase-junit4-reporter` v4.1.34
- [ ] Old `qase-junit4` dependency removed
- [ ] Old `qase-junit4-aspect` dependency removed (if present)
...
- Ensure AspectJ weaver is configured (required for JUnit 4 listener registration)
...
- Ensure `import io.qase.junit4.Qase;` is used

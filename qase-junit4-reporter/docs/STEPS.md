# Test Steps in JUnit 4

This guide covers how to define and report test steps for detailed execution tracking in Qase using the JUnit 4 reporter.
...
### JUnit 4 Listener Not Registered

**Possible causes:**

- `QaseJunit4Aspects` is not handling listener registration via AspectJ
- AspectJ weaver is not configured as javaagent

**Solutions:**

1. Verify `QaseJunit4Aspects` is handling listener registration via AspectJ.
2. Ensure `aspectjweaver` is configured as javaagent (required for both `@Step` processing AND listener registration in JUnit 4).
3. Check that the reporter dependency is correctly included in your project.

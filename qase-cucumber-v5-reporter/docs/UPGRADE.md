# Upgrade Guide: Cucumber 5 Reporter

This guide covers migration from Qase Java Reporter v3 to v4 for the Cucumber 5 reporter.
...
1. **Maven artifact ID renamed**: `qase-cucumber5-jvm` → `qase-cucumber-v5-reporter`
...
5. **New `Qase` utility class**: `import io.qase.cucumber5.Qase` for attachments and comments
...
**Gradle Before (v3):**
```groovy
testImplementation 'io.qase:qase-cucumber5-jvm:3.2.1'
```

**Gradle After (v4):**
```groovy
testImplementation 'io.qase:qase-cucumber-v5-reporter:4.1.33'
```
...
| `import io.qase.api.services.Attachments;` | Use `io.qase.cucumber5.Qase` instead |
...
- **v3:** `QaseEventListener implements Formatter` (Cucumber 5 API: `cucumber.api.event.*`)
- **v4:** `QaseEventListener implements ConcurrentEventListener` (Cucumber 5 API: `io.cucumber.plugin.event.*`)
...
- [ ] Maven/Gradle dependency updated from `qase-cucumber5-jvm` to `qase-cucumber-v5-reporter` v4.1.33
- [ ] Old `qase-cucumber5-jvm` dependency removed
...
- Ensure `import io.qase.cucumber5.Qase;` is used
...
- Verify Cucumber version compatibility (Cucumber 5.x)
...
- Ensure you're using qase-cucumber-v5-reporter version 4.1.5 or later

# Upgrade Guide: Cucumber 7 Reporter

This guide covers migration from Qase Java Reporter v3 to v4 for the Cucumber 7 reporter.
...
1. **Maven artifact ID renamed**: `qase-cucumber7-jvm` → `qase-cucumber-v7-reporter`
...
5. **New `Qase` utility class**: `import io.qase.cucumber7.Qase` for attachments and comments
...
**Gradle Before (v3):**
```groovy
testImplementation 'io.qase:qase-cucumber7-jvm:3.2.1'
```
...
| `import io.qase.api.services.Attachments;` | Use `io.qase.cucumber7.Qase` instead |
...
- **v4:** `QaseEventListener implements ConcurrentEventListener` (Cucumber 7 API: `io.cucumber.plugin.event.*`)
...
- [ ] Maven/Gradle dependency updated from `qase-cucumber7-jvm` to `qase-cucumber-v7-reporter` v4.1.34
- [ ] Old `qase-cucumber7-jvm` dependency removed
...
- Ensure `import io.qase.cucumber7.Qase;` is used
...
- Verify Cucumber version compatibility (Cucumber 7.x)
...
- Ensure you're using qase-cucumber-v7-reporter version 4.1.5 or later

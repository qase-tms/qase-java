# Upgrade Guide: Cucumber 4 Reporter

This guide covers migration from Qase Java Reporter v3 to v4 for the Cucumber 4 reporter.
...
1. **Maven artifact ID renamed**: `qase-cucumber4-jvm` → `qase-cucumber-v4-reporter`
...
5. **New `Qase` utility class**: `import io.qase.cucumber4.Qase` for attachments and comments
...
**Gradle Before (v3):**
```groovy
testImplementation 'io.qase:qase-cucumber4-jvm:3.2.1'
```

**Gradle After (v4):**
```groovy
testImplementation 'io.qase:qase-cucumber-v4-reporter:4.1.33'
```
...
| `import io.qase.api.services.Attachments;` | Use `io.qase.cucumber4.Qase` instead |
...
- **v3:** `QaseEventListener implements Formatter` (Cucumber 4 API: `cucumber.api.event.*`)
- **v4:** `QaseEventListener implements ConcurrentEventListener` (Cucumber 4 API: `io.cucumber.plugin.event.*`)
...
- [ ] Maven/Gradle dependency updated from `qase-cucumber4-jvm` to `qase-cucumber-v4-reporter` v4.1.33
- [ ] Old `qase-cucumber4-jvm` dependency removed
...
- Ensure `import io.qase.cucumber4.Qase;` is used
...
- Verify Cucumber version compatibility (Cucumber 4.x)
...
- Ensure you're using qase-cucumber-v4-reporter version 4.1.5 or later
